...
@Controller
@RequestMapping("/reverse_files")
public class ReverseFileController extends AbstractFileController {

    private final StorageFileService storageFileService;

    public ReverseFileController(StorageFileService storageFileService) {
    ...
    protected String getAllFiles(Model model) {
        model.addAttribute("files", storageFileService.loadAll(ConversationType.REVERSE)
                .map(path -> new FileData(path, ConversationType.REVERSE))
                .collect(Collectors.toList()));
        return "page/files/reverse_files";
    }

    @Override
    @GetMapping("/files/{filename:.+}")
    protected ResponseEntity<Resource> serveFile(@PathVariable String filename) {
        Resource file = storageFileService.loadAsResource(filename, ConversationType.REVERSE);
    ...
    protected String deleteFile(@PathVariable String filename) {
        storageFileService.delete(filename, ConversationType.REVERSE);
        return "redirect:/reverse_files";
    }
