...
@Controller
@RequestMapping("/findRoots_files")
public class FindRootsFileController extends AbstractFileController {

    private final StorageFileService storageFileService;

    public FindRootsFileController(StorageFileService storageFileService) {
    ...
    protected String getAllFiles(Model model) {
        model.addAttribute("files", storageFileService.loadAll(ConversationType.FINDROOTS)
                .map(path -> new FileData(path, ConversationType.FINDROOTS))
                .collect(Collectors.toList()));
        return "page/files/findRoots_files";
    }

    @Override
    @GetMapping("/files/{filename:.+}")
    protected ResponseEntity<Resource> serveFile(@PathVariable String filename) {
        Resource file = storageFileService.loadAsResource(filename, ConversationType.FINDROOTS);
    ...
    protected String deleteFile(@PathVariable String filename) {
        storageFileService.delete(filename, ConversationType.FINDROOTS);
        return "redirect:/findRoots_files";
    }
