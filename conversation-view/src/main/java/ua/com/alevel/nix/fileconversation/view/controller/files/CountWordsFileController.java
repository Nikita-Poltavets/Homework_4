...
@Controller
@RequestMapping("/countWords_files")
public class CountWordsFileController extends AbstractFileController {

    private final StorageFileService storageFileService;

    public CountWordsFileController(StorageFileService storageFileService) {
    ...
    protected String getAllFiles(Model model) {
        model.addAttribute("files", storageFileService.loadAll(ConversationType.COUNTWORDS)
                .map(path -> new FileData(path, ConversationType.COUNTWORDS))
                .collect(Collectors.toList()));
        return "page/files/countWords_files";
    }

    @Override
    @GetMapping("/files/{filename:.+}")
    protected ResponseEntity<Resource> serveFile(@PathVariable String filename) {
        Resource file = storageFileService.loadAsResource(filename, ConversationType.COUNTWORDS);
    ...
    protected String deleteFile(@PathVariable String filename) {
        storageFileService.delete(filename, ConversationType.COUNTWORDS);
        return "redirect:/countWords_files";
    }
